...
export default function Formulario() {
  ...
  const registerButton = async (e) => {
    ...
    try {
      ...
      if (res?.ok) {
        console.log(res);
        !isRegister && router.push("/redirigiendo");
        toast.success(
  ...
  const restablecerPass = async (e) => {
    ...
    try {
      const res = await axios.post("/api/restablecerPass", {
        email: form?.email,
      });
      toast.success(
  ...
  return (
    <>
      {show && (
        <Toaster key={2}>
          Correo electrónico de restablecimiento de contraseña enviado. Pronto
          recibirás un correo electrónico para restablecer tu contraseña. Si no
          lo encuentras, compruébala carpeta de correo no deseado y la papelera.
        </Toaster>
      )}
