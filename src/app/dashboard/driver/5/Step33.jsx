...
export default function Step33() {
  ...
  const clickCargaFormulario = () => {
    try {
      ...
      const array = formCarga?.driver5?.formulario3?.acciones;
      if (!array) {
        updateState({
          formCarga: {
            ...formCarga,
  ...
  useEffect(() => {
    setIsLoading(false);
    if (formCarga?.driver5?.formulario3) {
      setForm(formCarga.driver5.formulario3);
      const data = formCarga?.driver5?.formulario3?.acciones;
